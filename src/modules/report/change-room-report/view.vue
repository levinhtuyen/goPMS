  ...
  <pms-sections>
    <template #header>
      <div class="row q-mb-md">
        <div class="text-h5 text-bold">{{ t('change_room_report') }}</div>
      </div>
          ...
          <q-field placeholder="Select date time" dense stack-label filled @focus="onOpenDatePopUp">
            <template #control>
              <div
                ...
                tabindex="0"
              >
                {{ date.formatDate(String(dateRange.from), FORMAT_DATE_UI) }} -
                {{ date.formatDate(String(dateRange.to), FORMAT_DATE_UI) }}
              </div>
            </template>
            <template #prepend>
    ...
    <div class="relative-position">
      ...
      <div v-if="!isFetching">
        <q-table
          ...
          :rows="data?.changeRoomReportList"
          :columns="columns"
          hide-pagination
          hide-bottom
          row-key="no"
        >
          <template #body="props">
            <q-tr :props="props">
              ...
              <q-td key="oldRoomName" :props="props">
                {{ props.row.oldRoomName }}
              </q-td>

              <q-td key="newRoomName" :props="props">
                {{ props.row.newRoomName }}
              </q-td>
              ...
              <q-td key="reason" :props="props">
                <div class="slice-tooltip">
                  <div :data-title="props.row.reason">
                    {{ props.row.reason }}
                  </div>
                </div>
              </q-td>
              ...
              <q-td key="createTime" :props="props">
                <div>{{ newDateStamp(props.row.createTime, FORMAT_DATE_TIME_UI) }}</div>
                <div>{{ props.row.staffName }}</div>
              </q-td>
            </q-tr>
          </template>
        </q-table>
...
import {
  useChangeRoomReportQuery,
  IChangeRoomReportMutationVariables,
} from './apis/useChangeRoomReportQuery';
...
export default {
  setup() {
    ...
    const formValues = reactive<IChangeRoomReportMutationVariables>({
      page: 1,
      limit: 10,
    ...
    const { data, isFetching } = useChangeRoomReportQuery(formValues, true);
    ...
    const columns = computed(() => [
      ...
      { name: 'reason', align: 'left' as TAlignColumn, label: t('reason'), field: 'reason' },
      {
        name: 'createTime',
        align: 'left' as TAlignColumn,
        label: t('changed_at_by'),
        field: 'createTime',
      },
    ]);
