...
import { useEditPriceOfPricingPlanMutation } from '../apis/useEditPricingMutation';
...
const mutationEdit = useEditPriceOfPricingPlanMutation();
...
const {
  ...
  isFetching,
  refetch,
} = usePricingDetailQuery(pricingSn, {
  enabled: enabledPricingQuery.value,
});
const { notifyUpdateSuccess } = useNotify();
const setupPricingEditDialog = async (val: any) => {
  if (pricingSn.value === val.sn) {
    refetch.value();
  }
...
const moneyRules: ValidationRule<number>[] = [
  (value) => !!value || t('this_field_is_required'),
  (value) => (1 <= value && 100000000000 >= value) || t('the_value_is_from_1_to_100_000_000_000'),
];
// watch
watch(isFetching, () => {
  clearCol();
  ...
  pricingDetail.value?.priceOfTimeFrame &&
    pricingDetail.value.priceOfTimeFrame.forEach((val: any) => {
      if (val.bookingType === 1) {
        newHour.value.push(JSON.parse(JSON.stringify(val)));
      } else if (val.bookingType === 2) {
        if (val.priceType === 2) {
          newHourEarlyOv.value.push(JSON.parse(JSON.stringify(val)));
        } else {
          newHourLateOv.value.push(JSON.parse(JSON.stringify(val)));
        }
      } else if (val.bookingType === 3) {
        if (val.priceType === 2) {
          newHourEarlyDl.value.push(JSON.parse(JSON.stringify(val)));
        } else {
          newHourLateDl.value.push(JSON.parse(JSON.stringify(val)));
        }
      }
    });
  pricingDetail.value?.priceOfPricingPlan &&
    pricingDetail.value?.priceOfPricingPlan.forEach((val: any) => {
      if (val.bookingType === 2) {
        overnightFrame.value = JSON.parse(JSON.stringify(val.price));
      } else if (val.bookingType === 3) {
        dailyFrame.value = JSON.parse(JSON.stringify(val.price));
      }
    });
});
// watch
...
const onSave = () => {
  $q.dialog({
    message: t('are_you_sure_you_want_to_save_your_changes'),
    cancel: {
      color: 'grey-8',
      label: t('cancel'),
      noCaps: true,
  ...
  <q-dialog v-model="dialogVisible">
    <q-card style="padding: 0 20px 16px 20px; max-width: 1500px; width: 885px">
      <q-bar class="bg-white q-my-md q-mr-sm q-pa-none">
        <div class="text-h5 text-bold">{{ t('edit_pricing_plan') }}</div>
        <q-space />
      </q-bar>
      <q-separator />
        ...
        <q-form class="q-gutter-md" @submit.prevent.stop="onSave">
          <div class="row items-center">
            <div class="col q-mr-md">
              <p class="required q-ma-none text-weight-bold">{{ t('room_type') }}</p>
              <q-select
          ...
          <div class="row full-height">
            <div class="col-md-6 column block">
              ...
              >
                <q-tab :name="1" :label="t('hourly')" :class="isActiveTab(newHour?.length, 1)" />
                <q-tab :name="2" :label="t('overnight')" :class="isActiveTab(overnightFrame, 2)" />
                <q-tab :name="3" :label="t('daily')" :class="isActiveTab(dailyFrame, 2)" />
              </q-tabs>
            </div>
              ...
              <q-tab-panel :name="2">
                <div class="row full-height">
                  ...
                  <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                    ...
                    <div class="q-mt-md q-ma-sm">
                      <!-- revert text btn wrong because has been create bug for jira =)) -->
                      <q-btn
                        color="grey-4"
                        text-color="deep-orange"
                        no-caps
