...
const fetchFn = async (variables: IDeleteImportStockMutationVariables) => {
  const { data } = await axiosClient.post('/inventory/stock/removeImportStock', variables);
  return data;
};
  ...
  >(fetchFn, {
    onSuccess: (_, variables) => {
      ...
      queryClient.setQueryData<ResponseDto<any[]>>(QUERY_KEY.IMPORT_STOCK, () => ({
        ...prevCache,
        data: data.filter((importStock) => importStock.importStockSn !== variables.importStockSn),
      }));
    },
  });
};
